<script>
  import { FileText, Play, Copy, Download, Database, X } from 'lucide-svelte';
  ...
  async function checkGrammar() {
    ...
    try {
      ...
      if (inputMode === 'resource') {
        textToCheck = await fetchResourceText(selectedResourceUri, resourceAPI.getResource);
      }
    ...
    <div class="flex flex-col h-full">
      ...
      <div class="border-b border-gray-200 dark:border-gray-700 mb-4">
        <nav class="-mb-px flex space-x-8">
          <button
            ...
            class="{inputMode === 'text' ? 'border-red-500 text-red-600 dark:text-red-400' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300'} whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm transition-colors"
          >
            Text Input
          </button>
          <button
      ...
      <div class="flex-1 mb-4">
        {#if inputMode === 'text'}
          <textarea
            ...
            class="w-full h-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent resize-none bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400"
          ></textarea>
        {:else}
          <ResourceSelector 
            bind:selectedResourceUri
            label="Select a resource to check grammar"
      ...
      >
        {#if isProcessing}
          ...
          {inputMode === 'resource' ? 'Loading & Checking...' : 'Checking Grammar...'}
        {:else}
          <Play size={16} class="mr-2" />
          {inputMode === 'resource' ? 'Check Grammar from Resource' : 'Check Grammar'}
        {/if}
      </button>
    </div>
    ...
    <div class="flex flex-col h-full">
      ...
      {:else if outputText}
        <textarea
          ...
          class="flex-1 min-h-0 w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm font-mono resize-none focus:ring-2 focus:ring-red-500 focus:border-transparent mt-4"
          readonly
        ></textarea>
      {:else}
        <div class="flex-1 w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-800">
